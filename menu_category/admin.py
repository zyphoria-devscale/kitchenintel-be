...
class MenuCategoryForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['parent_id'].queryset = MenuCategory.objects.filter(parent_id__isnull=True)
        
        # If we're editing an existing object, exclude it from the parent options
        if self.instance.pk:
            self.fields['parent_id'].queryset = self.fields['parent_id'].queryset.exclude(pk=self.instance.pk)
